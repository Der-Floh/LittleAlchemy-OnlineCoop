...
// screenshots of the co-op UI.  Usage: node scripts/ui-preview.ts <outDir>
import path from 'node:path';
import fs from 'node:fs';
import { launchPlayer, closePlayer, openGame, dismissLoadingScreen } from '../test/e2e/helpers.ts';
...
try {
  ...
  await page.evaluate(() => {
    const { panel, adapter } = window.__laCoop!;
    panel.toast({ image: adapter.elementInfo(5).image, parts: [{ who: { name: 'Bob', color: 1 } }, ' discovered ', { el: 'steam', bold: true }] });
  });
