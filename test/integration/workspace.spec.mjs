...
// All ops emitted so far, flattened.
const sentOps = async () => (await recorded(page, 'ops')).flatMap((batch) => batch.ops);
const remote = (ops, by = BOB) => page.evaluate(([o, b]) => window.__laCoop.bridge.applyOps(o, b), [ops, by]);
// Where a shared position lands on this screen (same maths as projection.js).
const projected = (x, y) =>
  page.evaluate(([sx, sy]) => {
...
test('moving an element sends hold, moves and release, ending where it landed', async () => {
  ...
  expect(Math.abs(landed.left - element.left)).toBeLessThanOrEqual(1);
  expect(Math.abs(landed.top - element.top)).toBeLessThanOrEqual(1);
});

test('a combination is one batch: the result is added and the ingredient removed', async () => {
  await dragLibraryToWorkspace(page, E.water, 500, 400);
  await expect.poll(sentOps).toHaveLength(1);
  const [[, waterOid]] = await sentOps();
  await dragLibraryToWorkspace(page, E.fire, 500, 400); // fire from the library onto the water
  await expect.poll(async () => (await recorded(page, 'ops')).length).toBe(2);
  const batch = (await recorded(page, 'ops'))[1].ops;
...
test('combining two canvas elements deletes both in the same batch as the result', async () => {
  ...
  const [[, waterOid], [, fireOid]] = await sentOps();
  ...
  const batches = await recorded(page, 'ops');
  const last = batches.at(-1).ops;
  expect(last.filter((o) => o[0] === 'd').map((o) => o[1]).sort()).toEqual([waterOid, fireOid].sort());
...
test('in a room, the clear button removes only my own elements', async () => {
  ...
  await expect.poll(async () => (await recorded(page, 'ops')).at(-1)).toEqual({ ops: [['d', mine]], clear: true });
});
...
test('reconcile replaces the canvas with the room’s, silently', async () => {
  ...
  await expect.poll(sentOps).toHaveLength(2);
  const before = (await recorded(page, 'ops')).length;
  await page.evaluate(() => {
    const { bridge, sync } = window.__laCoop;
    const State = sync.state.constructor;
  ...
  await page.waitForTimeout(150);
  expect((await recorded(page, 'ops')).length).toBe(before);
});
