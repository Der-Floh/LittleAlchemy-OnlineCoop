...
const apply = (tuples, meta = { by: { id: 'x', name: 'Remote', color: 1 }, sync: false }) =>
  page.evaluate(([t, m]) => window.__laCoop.adapter.applyTuples(t, m), [tuples, meta]);
...
test('a small batch goes through the game itself: library, counter and save', async () => {
  ...
  await expect.poll(() => recorded(page, 'applied')).toHaveLength(1);
  const [event] = await recorded(page, 'applied');
  ...
  expect(await recorded(page, 'local')).toEqual([]);
});
...
test('duplicates and pairs that are not recipes are ignored', async () => {
  ...
  expect(await savedPairs(page)).toEqual(['1+2']);
  expect(await recorded(page, 'applied')).toHaveLength(1);
});
...
test('a big sync is rebuilt in one go, but not while the player is dragging', async () => {
  ...
  const events = await recorded(page, 'applied');
...
test('local discoveries by drag and drop are reported once, with new elements', async () => {
  ...
  await waitForElement(page, E.steam);
  await expect.poll(() => recorded(page, 'local')).toHaveLength(1);
  const [event] = await recorded(page, 'local');
  ...
  expect(await recorded(page, 'local')).toHaveLength(1);
});
...
test('the pre-co-op backup restores the old save, library and counter', async () => {
  ...
  expect((await recorded(page, 'reset')).map((e) => e.reason)).toEqual(['backup-restored']);
...
test("the game's own reset empties the co-op state too", async () => {
  ...
  expect((await recorded(page, 'reset')).map((e) => e.reason)).toEqual(['game-reset']);
  expect(await progressCounter(page)).toBe('4/580');
});
