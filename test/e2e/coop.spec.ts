...
const t0 = Date.now();
const mark = (label: string) => console.log('  [' + ((Date.now() - t0) / 1000).toFixed(1) + 's] ' + label);
...
test('two players share discoveries live, survive a reload and a host change', async () => {
  ...
  // EXTENSION_DIR_B: Bob runs another build (e.g. the previous release), to check that both can share a room.
  const bob = await launchPlayer('bob', { extensionDir: process.env.EXTENSION_DIR_B });
  try {
    await Promise.all([openGame(alice), openGame(bob)]);
    const version = (page: Page) => page.evaluate(() => window.__laCoop!.version);
    mark('launched + game open (Alice ' + (await version(alice.page)) + ', Bob ' + (await version(bob.page)) + ')');
...
test('invite links, one co-op tab per browser, and restoring the pre-co-op backup', async () => {
  ...
  try {
    ...
    await second.waitForFunction(() => window.__laCoop?.panel.available === 'passive', null, { timeout: 90_000 });
