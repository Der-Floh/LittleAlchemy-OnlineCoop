...
import { launchPlayer, closePlayer, openGame, dismissLoadingScreen } from './helpers.mjs';

const state = (page) => page.evaluate(() => window.__laCoop.session.state);
const playerId = (page) => page.evaluate(() => window.__laCoop.session.playerId);
const memberCount = (page) => page.evaluate(() => window.__laCoop.session.members.length);
...
async function hostAction(page, targetId, label) {
  await openPanel(page);
