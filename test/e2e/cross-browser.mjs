...
const run = (driver, fn, ...args) => driver.executeScript(`return (${fn}).apply(null, arguments)`, ...args);

async function until(driver, fn, args, what, timeout = 60_000) {
...
async function coopUi(driver) {
  const host = await driver.findElement(By.css('#la-coop-root'));
  return host.getShadowRoot();
}

async function firefoxDrag(driver, elementId, x, y) {
...
async function firefoxCombine(driver, a, b, x = 520, y = 420) {
  await firefoxDrag(driver, a, x, y);
  await sleep(200);
  await firefoxDrag(driver, b, x, y);
}

const alice = await launchPlayer('alice-chrome');
let bob = null;
try {
  ...
  await until(bob, () => window.__laCoop && window.__laCoop.panel && window.__laCoop.panel.available === 'ready', [], 'co-op to start in Firefox', 90_000);
  await run(bob, () => {
  ...
  await until(bob, (id) => window.game.progress.includes(id), [E.steam], 'steam to reach Firefox', 30_000);
  const toast = await run(bob, () => {
    const t = document.getElementById('la-coop-root').shadowRoot.querySelector('.toast');
    return t ? t.textContent : null;
  });
  if (!toast || !toast.includes('discovered steam')) throw new Error('No toast in Firefox, got: ' + toast);
  ...
  const pairs = (json) =>
    JSON.parse(json)
      .parents.map((p) => Math.min(...p) + '+' + Math.max(...p))
      .sort()
      .join(',');
