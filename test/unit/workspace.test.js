...
import { parseOps, parseSnapshot, MAX_OPS_PER_BATCH, MAX_ELEMENTS } from '../../src/workspace/ops.js';
import { WorkspaceState } from '../../src/workspace/state.js';
import { toShared, toLocal, pointToShared, pointToLocal } from '../../src/workspace/projection.js';
...
test("holds protect an element from other players, not from its holder", () => {
  ...
  for (const op of [['m', 'e1', 0.1, 0.1], ['d', 'e1'], ['h', 'e1']]) {
...
test('ids cannot be reused and the canvas has a cap', () => {
  ...
  const adds = Array.from({ length: MAX_ELEMENTS }, (_, i) => ['a', 'f' + i, 1, 0, 0]);
...
test('snapshots round-trip and the hash ignores insertion order', () => {
  ...
  const s2 = WorkspaceState.fromSnapshot(parseSnapshot(s1.snapshot()));
...
test('client prediction converges with the host after a rejected batch', () => {
  ...
  assert.ok(host.applyBatch(pending[0][1], B, { authoritative: true }).ok);
  assert.equal(host.applyBatch(pending[1][1], B, { authoritative: true }).ok, false);
  ...
  assert.ok(host.applyBatch(pending[2][1], B, { authoritative: true }).ok);
...
test('positions round-trip exactly on the same screen', () => {
  for (const [left, top] of [[0, 0], [123, 456], [1396, 826], [700, 13]]) {
