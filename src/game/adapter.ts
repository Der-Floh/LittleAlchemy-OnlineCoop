...
import { normalizePair, pairKey, parseTuple, tupleKey, tuplesFromHistory, type Tuple } from '../sync/pairs.ts';
import type { ApplyMeta, GameLike } from '../net/session.ts';
...
export type ElementInfo = { id: number; name: string; image: string | null };
export type Recipe = { tuple: Tuple; children: number[] };
export type BackupInfo = { savedAt: number; count: number };
type BackupRecord = BackupInfo & { progress: string | null; achievements: string | null };
type PendingBatch = { tuples: Tuple[]; meta: ApplyMeta };

export type AdapterEvents = {
    // The player discovered a recipe themselves.
    local: { tuple: Tuple; children: number[]; newElements: number[] };
    // Discoveries from the room reached the game.
    applied: { meta: ApplyMeta; recipes: Recipe[]; newElements: number[]; bulk: boolean };
    reset: { reason: 'game-reset' | 'backup-restored' };
};

const GAME_FUNCTIONS = ['initProgress', 'checkIfNotAlreadyDone', 'getFinalElements', 'changeProgressCounter'] as const;

const REQUIRED_FEATURES: [name: string, check: (w: Window) => boolean][] = [
    ['jQuery', (w) => typeof w.jQuery === 'function'],
    ['game', (w) => !!w.game && GAME_FUNCTIONS.every((fn) => typeof w.game[fn] === 'function')],
...
export class GameAdapter extends Emitter<AdapterEvents> implements GameLike {
    readonly win: Window;
    ...
    isReady(): boolean {
        ...
        return (
            safe(() => {
                ...
                const list = w.loadingScreen?.list;
                return Array.isArray(list) ? list.includes('libraryShowed') : w.library.el.children.length > 0;
            }) ?? false
        );
    }

    whenReady({ timeoutMs = 90_000, intervalMs = 100 } = {}): Promise<void> {
    ...
    start(): void {
        ...
        $doc.on('updateHistory' + NS, (_event, pair: unknown, date: unknown) => this._onUpdateHistory(pair, date));
    ...
    private _resync(): void {
        ...
        this._keys = new Set(this._tuples.map((tuple) => tupleKey(tuple)));
    }

    private _onUpdateHistory(pair: unknown, date: unknown): void {
        if (this._applying || !Array.isArray(pair)) return;
        const list: readonly unknown[] = pair;
        const normalized = normalizePair(list[0], list[1]);
        if (!normalized) return;
        const key = pairKey(normalized[0], normalized[1]);
        if (this._keys.has(key)) return;
        const tuple: Tuple = [normalized[0], normalized[1], typeof date === 'number' ? date : Date.now()];
        this._keys.add(key);
    ...
    applyTuples(tuples: readonly unknown[], meta: ApplyMeta = { by: null, sync: false }): Tuple[] {
        const added: Tuple[] = [];
    ...
    private _applyByTrigger(batch: PendingBatch): void {
        const w = this.win;
        const game = w.game;
        const $doc = w.jQuery(w.document);
        ...
        try {
            for (const tuple of batch.tuples) {
                ...
                const children = this.childrenOf(tuple);
    ...
    private _applyBulk(batches: PendingBatch[]): void {
        const w = this.win;
        const game = w.game;
        const before = new Set(this._allElements());
        const done = new Set(game.history.parents.map((p) => pairKey(Math.min(p[0] ?? 0, p[1] ?? 0), Math.max(p[0] ?? 0, p[1] ?? 0))));
    ...
    private _rebuildGame({ clearWorkspace }: { clearWorkspace: boolean }): void {
        const w = this.win;
        const game = w.game;
        ...
        if (achievements?.data && typeof achievements.initialCheck === 'function') {
    ...
    childrenOf(pair: readonly [number, number, ...unknown[]]): number[] {
    ...
    private _allElements(): number[] {
        const game = this.win.game;
        return [...game.prime, ...game.progress, ...(game.hiddenElements ?? [])];
    }

    elementCount(): { have: number; total: number } {
        const game = this.win.game;
        return { have: game.progress.length + game.prime.length, total: game.maxProgress ?? 0 };
    }

    elementInfo(id: number): ElementInfo {
        const bases = this.win.bases;
        const name = bases.names?.[id] || '#' + id;
        const image = bases.images?.[id] ? 'data:image/png;base64,' + bases.images[id] : null;
    ...
    backupInfo(): BackupInfo | null {
        ...
        if (!raw) return null;
        const data = safe(() => JSON.parse(raw) as BackupRecord | null);
    ...
    ensureBackup(): boolean {
        ...
        ls.setItem(BACKUP_KEY, JSON.stringify(record));
        return true;
    }
    ...
    restoreBackup(): void {
        ...
        const data = JSON.parse(raw) as BackupRecord;
        const history = (data.progress ? JSON.parse(data.progress) : { parents: [], date: [] }) as Partial<LAHistory> | null;
        ...
        w.game.history = history as LAHistory;
        this._applying = true;
        try {
        ...
        if (data.achievements && w.achievements) {
            ...
            const earned = safe(() => JSON.parse(achievements) as unknown);
